...
function getStatusLabel(status) {
  switch (status) {
    ...
    case "confirmed": return "قيد المعالجة";
...
router.put("/:id/status", async (req, res) => {
  ...
  try {
    ...
    // تحديث حالة الطلب
    let timeField = null;
    if (status === "confirmed" || status === "processing") timeField = "processing_at";
    ...
    // القيود المحاسبية فقط عند قيد التوصيل — الفشل يُسجَّل ولا يمنع تحديث الحالة
    if (status === "delivering") {
      const journalResult = await postDeliveringJournals(conn, orderId, req);
      await upsertJournalJob({
        sourceType: "order",
        sourceId: orderId,
        orderNumber: journalResult.orderNumber || orderId,
        status: journalResult.ok ? "posted" : "failed",
        errorCode: journalResult.errorCode || null,
        errorMessage: journalResult.ok
          ? null
          : (journalResult.reasons || []).join(" — "),
      });
      if (!journalResult.ok) {
        console.error(
          "DELIVERING JOURNALS ERROR:",
          journalResult.reasons?.join(" — ") || "unknown"
        );
      }
    }
