...
router.get("/", async (req, res) => {
  try {
    ...
    console.error("GET PAYMENT TYPES ERROR:", err?.message || err);
...
router.post("/", async (req, res) => {
  try {
    ...
    const [[codesRow]] = await db.query(
      "SELECT COALESCE(MAX(code), 0) AS maxCode FROM payment_types"
    );

    const [[sortRow]] = await db.query(
      "SELECT COALESCE(MAX(sort_order), 0) AS maxSortOrder FROM payment_types WHERE branch_id = ?",
      [branch_id]
    );
    ...
    console.error("ADD PAYMENT TYPE ERROR:", err?.message || err);
    res.status(500).json({
      success: false,
...
router.put("/:id", async (req, res) => {
  try {
    ...
    console.error("UPDATE PAYMENT TYPE ERROR:", err?.message || err);
    res.status(500).json({ success: false });
  }
});
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    console.error("DELETE PAYMENT TYPE ERROR:", err?.message || err);
    res.status(500).json({ success: false });
  }
});
